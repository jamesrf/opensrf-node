...
osrfJSONParser.prototype.jsIterate = function (arg, callback) {
  if (arg && typeof arg === 'object') {
    if (arg.constructor === Array) {
      for (var i = 0; i < arg.length; i++) { callback(arg, i) }
    } else if (arg.constructor === Object) {
      for (var i in arg) { callback(arg, i) }
...
osrfJSONParser.prototype.jsClone = function (obj) {
  ...
  if (obj.constructor === Array) {
    return obj.map((v) => self.jsClone(v))
  }
...
osrfJSONParser.prototype.encodeJS = function (arg) {
  ...
  if (arg._isfieldmapper) {
    var newarr = []
    if (!arg.a) arg.a = []
    for (var i = 0; i < arg.a.length; i++) { newarr[i] = this.encodeJS(arg.a[i]) }
  ...
  if (arg.length !== undefined) {
    newobj = []
    for (var i = 0; i < arg.length; i++) { newobj.push(this.encodeJS(arg[i])) }
    return newobj
  }

  newobj = {}
  for (var i in arg) { newobj[i] = this.encodeJS(arg[i]) }
  return newobj
}
