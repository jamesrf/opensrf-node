...
osrfStack.prototype.push = function (net_msg, callbacks) {
  ...
  if (!osrf_msgs) {
    try {
      ...
      console.error('Error parsing OpenSRF message body as JSON: ' + net_msg.body + '\n' + E)
      ...
      try {
        ...
        console.error('Unable to clean up message, giving up: ' + net_msg.body)
        return
      }
    }
  }
  ...
  for (var i = 0; i < osrf_msgs.length; i++) { this.queue.push({ msg: osrf_msgs[i], ses: ses }) }
...
osrfStack.prototype.handleMessage = function (ses, osrf_msg) {
  ...
  if (osrf_msg.type() == constants.OSRF_MESSAGE_TYPE_STATUS) {
    ...
    if ((status + '').match(/^4/) || (status + '').match(/^5/)) {
      if (req && req.onmethoderror) { return req.onmethoderror(req, status, status_text) }
    }
  }
